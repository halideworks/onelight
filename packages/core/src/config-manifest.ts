...
export const CONFIG_VARS: readonly ConfigVar[] = [
  ...
  {
    name: "ONELIGHT_SERVER_URL",
    scope: ["worker"],
    subsystem: "worker",
    kind: "url",
    required: true,
    summary: "Where the worker claims jobs and reports results.",
    doc: "Workers pull: the server no longer dials them, so this is the only address in the protocol and it is configuration, never something a job payload can name. Use the internal address of the server container, not PUBLIC_URL.",
    example: "http://onelight:3000",
    compose: "interpolate",
    composeDefault: "http://onelight:3000",
  },
  {
    name: "WORKER_SECRET",
    scope: ["server", "worker"],
    ...
    kind: "string",
    secret: true,
    summary: "Signs a worker's claim, and mints the token that scopes it.",
    doc: "Set it on the server to enable media processing, and on every worker that may claim from it.",
  ...
  },
  {
    ...
    doc: "Policy, so it is set on the server and travels to the worker inside the job it hands out: a worker that runs past the ceiling gives the job up and it is retried, rather than holding a lease forever on a job nobody is watching. The worker heartbeats the lease while it works, so a long encode does not expire mid-run.",
    commented: true,
    compose: "interpolate",
  },
  {
...
export const CONFIG_GROUPS: readonly ConfigGroup[] = [
  ...
  },
  {
    name: "media worker",
    subsystem: "worker",
    members: ["ONELIGHT_SERVER_URL", "WORKER_SECRET"],
    severity: "error",
    message:
      "ONELIGHT_SERVER_URL and WORKER_SECRET must be set together, or the worker has nowhere to claim jobs from and nothing to sign a claim with.",
  },
  {
    name: "mail",
    subsystem: "mail",

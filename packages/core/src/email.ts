...
import { constantTimeEqual, hmacSha256Hex, utf8 } from "./crypto.js";
...
/* ---- the headers that decide how mail behaves ----

   Three things a client does with a message have nothing to do with its body.

   THREADING. Five notes about one asset should arrive as one conversation, and
   a mail client decides that from Message-ID and References. Keyed on the thing
   the notes are about, so the thread is "this asset" rather than "this hour".

   UNSUBSCRIBING. Gmail and Outlook both look for List-Unsubscribe, and with
   List-Unsubscribe-Post they show a one-click control instead of leaving people
   to hunt for it or to report the message as spam, which is what actually
   damages delivery. The URL has to accept an unauthenticated POST, so it
   carries its own signed token.

   PRECEDENCE. Auto-replies and out-of-office messages should not answer a
   notification. Auto-Submitted says so in the standard way.

   The domain in an id is not a hostname to connect to; it only has to be
   stable and ours, so it is derived from the public URL. */
export const messageIdFor = (publicUrl: string, key: string): string => {
  let host = "onelight.invalid";
  try {
    host = new URL(publicUrl).host || host;
  } catch {
    /* A malformed public URL must not stop the mail going out. */
  }
  return `<${key.replace(/[^A-Za-z0-9._-]/g, "-")}@${host}>`;
};
...
export interface MailHeaderOptions {
  ...
  messageKey: string;
  /** The conversation it belongs to, if it belongs to one. */
  threadKey?: string | undefined;
  /** Where one click of "unsubscribe" should POST. */
  unsubscribeUrl?: string | undefined;
}

export const mailHeaders = (
  options: MailHeaderOptions,
): Record<string, string> => {
  const headers: Record<string, string> = {
    "Message-ID": messageIdFor(options.publicUrl, options.messageKey),
    /* Generated by a system, and no auto-reply should answer it. */
    "Auto-Submitted": "auto-generated",
    "X-Auto-Response-Suppress": "All",
  };
  if (options.threadKey) {
    const thread = messageIdFor(options.publicUrl, options.threadKey);
    headers["References"] = thread;
    headers["In-Reply-To"] = thread;
  }
...
/* ---- the authority in an unsubscribe link ----

   A mail client POSTs the List-Unsubscribe URL with no cookies at all, so the
   link has to carry its own authority. It is the user id with a signature over
   it, and the only thing it authorises is turning email off: not reading
   anything, not changing anything else.

   Signed here rather than in the API or the sweep, because both need it and two
   implementations of one signing scheme is how they drift apart. */
export const unsubscribeToken = async (
  secretKey: string,
  userId: string,
): Promise<string> => {
  const signature = await hmacSha256Hex(secretKey, `unsubscribe:${userId}`);
  return `${userId}.${signature.slice(0, 32)}`;
};

/** Whose token this is, or null if nobody signed it. */
export const unsubscribeSubject = async (
  secretKey: string,
  token: string,
): Promise<string | null> => {
  ...
  const expected = (
    await hmacSha256Hex(secretKey, `unsubscribe:${userId}`)
  ).slice(0, 32);
  return constantTimeEqual(utf8(given), utf8(expected)) ? userId : null;
};

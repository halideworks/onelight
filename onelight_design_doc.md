...
- **Reference startup**: after the native first frame arrives, Automatic mode prepares the feature-gated decoder, bounded initial runway, and 1x audio clock behind the visible native picture. Selecting Reference reuses that preparation and pauses native playback only for the final exact-frame handoff. Explicit Native mode performs no background decode. Preparation and click-to-handoff latency are reported through the privacy-bounded playback diagnostic. *(Added 2026-07-24 after a cold staging source reached the old five-second open timeout.)*
- **Reference scrub**: the playhead follows the pointer at the display tick while Reference decode runs latest-target-wins. One bounded decode operation is allowed in flight, forward drags reuse the packet iterator and copy only the requested plane, reverse movement resets from the verified key packet, and release settles on the exact integer frame. The 4K renderer alternates persistent YUV texture banks to avoid overwriting textures still used by the previous draw. *(Added 2026-07-24.)*
- **Transport**: J/K/L with 2×/4×/8× shuttle both directions, space, ←/-> ±1 frame, Shift±10, I/O range marks + loop, playback rate 0.25-2×. Normal playback preserves pitch. Forward shuttle uses direct resampling for audible varispeed sound. *(Superseded 2026-07-23: pitch preservation during shuttle routed non-1× audio through browser time-stretch processors that produced advancing picture with silent audio on a verified Chrome/Firefox, macOS/Windows setup. Direct resampling avoids that path and matches an editor's expected shuttle sound.)*
...
- Rate limits on auth, share-passphrase attempts, and anonymous comment posting; audit log of auth/share/download events; webhook signatures HMAC-SHA256 with retries/backoff (frame.io parity). *(Hardened 2026-07-23: each delivery carries a signed Unix timestamp and signs `timestamp.body`, delivery claims are atomic, response reads are bounded, and the timeout covers headers plus body consumption.)*
...
- *(Superseded 2026-07-23 for the v1 public contract: Onelight accepts at most 128 GiB per upload, 8,192 parts, and 5 GiB per part on every backend. Completion requires one contiguous, unique part sequence whose persisted byte total exactly matches the declared upload size. These deliberately lower cross-backend limits keep Node, D1, local disk, S3 and R2 behavior identical.)*
...
- *(Hardened 2026-07-23: authenticated event-stream requests remain open, wake immediately on in-process publishes, send a 15 second heartbeat for cross-isolate progress, and preserve `Last-Event-ID` replay. Presence writes are throttled to one durable update per viewer every five minutes.)*
